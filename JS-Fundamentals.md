...
Не получает свой `thi` keyword как метод, и использует кейворд функции в которую вложен, а если нет - глобальный window. При этом если с помощью кейворда обратиться к свойству обьекта с стрингом - вернёт пустой стринг, а в других случаях - `undifined`. И не получает свой обьект с аргументами, как это делают другие функции в `Execution context` 
