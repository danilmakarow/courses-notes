...
Можно менять типы данных между собой - только первые три. Функции для этого - в отдельной вкладке. Так же js меняет типы данных автоматически: `const jonas =
"i'm " + 45 + "and another im " + 23;` При выводе этого должна произойти ошибка - Number & String нельзя приплюсовать оперетором `+`. По одиночке - оператор сработает. Но когда добавляются два разных типа - js автоматом меняет 23 c Number на String, что бы вывести данные. Тригером для этого является оператор `+`. То есть при выводе данных + будет триггером для конвертирования всего в String. Будет работать и в обратную сторону, но с оператором `-`. `console.log('23'-'10'-3);` Тут автоматом все преобразуется в цифри, и результат выйдет `10`. В других языках нужно было бы в ручную указывать смену типа данных - ` + String(23);` / `Number('23')`, но не в джаве. Наоборот не сработает - минусовать стринги от намберов и поюсовать цифры в стрингах не выйдет. Тут уже нужно будет менять вручную. 
